...
namespace Gameplay.Effect
{
    public class ModifierManager : MonoBehaviour
    {
        ...
        private readonly Dictionary<GameplayTag, List<ModifierInstance>> _modifiers = new();
        ...
        public void AddModifiers(List<ModifierConfig> permanentModifiers, EffectInstance effect)
        {
            ...
            foreach (ModifierConfig mod in permanentModifiers)
            {
                ...
                if (!_modifiers.TryGetValue(mod.AttributeTag, out List<ModifierInstance> activeModifiers))
                {
                    // 不存在：创建新列表
                    activeModifiers = new List<ModifierInstance>();
                    _modifiers.Add(mod.AttributeTag, activeModifiers);
                }
                activeModifiers.Add(new ModifierInstance()
                {
                    Config = mod,
                    SourceEffect = effect,
        ...
        {
            if(_modifiers.TryGetValue(attribute,out List<ModifierInstance> activeModifiers))
            {
